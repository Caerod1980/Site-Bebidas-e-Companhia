...
async function loadProfile() {
  ...
  document.getElementById('playerName').innerText =
    currentUser.name || 'Jogador Bebcom';
  ...
  document.getElementById('rewardText').innerText =
    currentUser.stats?.rewardsUnlocked || 0;
...
async function loadMissions() {
  ...
  box.innerHTML = data.missions.map(mission => `
    <div class="mission">
      <h4>${mission.title}</h4>
      <p class="small">${mission.description || ''}</p>
      <p class="small">Meta: ${mission.targetXp || 100} XP</p>
      <p class="small">Prêmio: ${mission.rewardTitle || 'Recompensa Bebcom'}</p>
    </div>
  `).join('');
}
